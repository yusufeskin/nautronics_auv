...
KP = 0.4
TOLERANCE_RAD = math.radians(3.0)  # 3 degree tolerance
MIN_SPEED = 0.0
...
class AbsoluteYawActionServer(Node):
    def __init__(self):
        super().__init__("absolute_yaw_action_server")
        ...
        self.get_logger().info("Absolute yaw action ready and listening /current_attitude")

    def attitude_callback(self, msg):
        # msg.z = yaw in degrees
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info(f"accepted: absolute target:{goal_request.target_angle_deg}°")
    ...
    async def execute_callback(self, goal_handle):
        # The goal is treated as an ABSOLUTE angle in degrees
        target_yaw_abs = math.radians(goal_handle.request.target_angle_deg)
        target_yaw_abs = normalize_angle(target_yaw_abs)
        ...
        while rclpy.ok():
            ...
            feedback_msg.current_angle_deg = math.degrees(self.current_yaw)
        ...
        result.success = True
        result.message = f"Reached absolute target {goal_handle.request.target_angle_deg}."
...
def main(args=None):
    ...
    action_server = AbsoluteYawActionServer()
