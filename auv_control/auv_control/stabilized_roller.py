...
TOLERANCE_RAD = 0.1
BASE_DOWNWARD_THRUST = 0.25 # Dünya ekseninde aracı aşağı bastıracak itki miktarı. Aracın pozitif yüzerliğine göre ayarlayabilirsiniz.
...
class StabilizedRollActionServer(Node):
    def __init__(self):
        super().__init__("stabilized_roll_action_server")

        self.callback_group = ReentrantCallbackGroup()
        self._action_server = ActionServer(
            self,
            Roll,
            "roll", # Orijinal ağacı (sami_gate.py) bozmamak için ismi "roll" bıraktık. (Eski roller yerine bunu çalıştıracaksınız)
        ...
        self.get_logger().info("Stabilized Roll action ready and listening /current_attitude")
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info(f"accepted: target:{goal_request.target_angle_deg}° with stabilization")
    ...
    async def execute_callback(self, goal_handle):
            ...
            while rclpy.ok():
                ...
                # 1. Roll (Açısal Hız) Komutu
                cmd.angular.x = max_speed * direction
                
                # 2. Dinamik İtki Komutu (Dünya ekseninde hep -Z'ye basması için Gövde Y ve Z itkilerini hesaplıyoruz)
                # Not: math.cos ve math.sin radyan cinsinden çalışır, current_roll zaten radyandır.
                cmd.linear.z = -BASE_DOWNWARD_THRUST * math.cos(self.current_roll)
                cmd.linear.y = -BASE_DOWNWARD_THRUST * math.sin(self.current_roll)
            ...
            result.success = True
            result.message = f"{target_deg_relative} derece dönüldü ve derinlik korundu."
            return result
...
def main(args=None):
    ...
    action_server = StabilizedRollActionServer()
