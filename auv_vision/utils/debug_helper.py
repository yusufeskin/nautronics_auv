...
def draw_debug_pnp(frame, results, detections_msg: DetectionArray, model_type: str) -> np.ndarray:
    debug_frame = frame.copy()
    dets = detections_msg.detections
    ...
    if model_type in ('bbox', 'keypoint'):
        for det in dets:
            ...
            pnp_str = f" D:{det.distance:.2f}m Y:{det.yaw_angle:.1f}" if getattr(det, 'distance', -1.0) > 0 else ""
            
            if is_ghost:
                label = f"GHOST ID:{track_id} {det.class_name}{pnp_str}"
            else:
                if track_id != -1:
                    label = f"ID:{track_id} {det.class_name} {det.confidence:.2f}{pnp_str}"
                else:
                    label = f"{det.class_name} {det.confidence:.2f}{pnp_str}"
